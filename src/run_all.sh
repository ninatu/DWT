...
for dataset in $files; do
    ...
    echo "${dataset} | ${dtw} | ${pruned_dtw} | $(echo $dtw / $pruned_dtw | bc -l)" >> ${outfile}
    echo "$dtw_sum  $pruned_dtw_sum"
done

echo ============================== >> ${outfile}
echo "SUM | $dtw_sum | $pruned_dtw_sum | $(echo $dtw_sum / $pruned_dtw_sum | bc -l)" >> ${outfile}
